...
-- This script trains the LeNet-5 architecture
-- on MNIST dataset with Heurisic Initialization and 50% negative MNIST. Testing can be performed by
-- providing the pretrained network as the first
...
function loadDataset(flag)
	...
	print('Inverting 50% of trainset images...')
...
function createModel(loadExisting)
	...
	if loadExisting ~= nil then
		...
		-- To use the Heuristic weight initialization
		-- the 'weight-init.lua' file must be present in 
		-- the same folder

		-- Heuristic Weight Initialization is based on
		-- "Efficient backprop"
		-- Yann Lecun, 1998
		...
		-- Initialise weights using Heuristic model 
		net = require('weight-init')(net, method)
...
function training(loadExisting)
	...
	if loadExisting == nil then
		...
		-- Save Plot as PNG
		gnuplot.pngfigure('LeNet-5_with-50%-Negative-MNIST.png')
		...
		-- Save the network.
		torch.save('LeNet-5_with-50%-Negative-MNIST.t7',net)
	end
end
