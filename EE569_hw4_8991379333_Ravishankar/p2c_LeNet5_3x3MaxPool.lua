...
-- This script modifies the 2x2 Max-pooling layer of LeNet-5
-- with 3x3 Max-pooling and 2 pixel stride. The number of inputs
-- to the first fully-connected layer is also modified.
...
function TranslatedDataset(images)
	...
	for i = 1,images:size(1) do
		...
		images[i] = image.translate(images[i],torch.random(-6,6),torch.random(-6,6))
...
function createModel(loadExisting)
	...
	if loadExisting ~= nil then
		...
		-- Max-pooling layer with 3x3 window tih stride 2
		...
		-- reshapes from a 3D tensor of 16x4x4 into 1D tensor of 16*4*4
		net:add(nn.View(16*4*4))

		-- Dense layer with 16*4*4 inputs and 120 neurons
...
function training(loadExisting)
	...
	if loadExisting == nil then
		...
		-- Save Plot as PNG
		gnuplot.pngfigure('LeNet-5_3x3MaxPool.png')
		...
		-- Save the network.
		torch.save('LeNet-5_3x3MaxPool.t7',net)
	end
end
