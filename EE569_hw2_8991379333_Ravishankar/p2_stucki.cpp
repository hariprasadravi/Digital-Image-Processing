		...
		for(int i=2;i<Sizeh-2;i++){	
			...
			if(i%2 == 0){														// check for even rows						
				...
				for(int j=2;j<Sizew-2;j++){
					...
					NewImagedata[i][j+1][k] += ((8*error)/42);							// diffusing error to unprocessed pixels
					NewImagedata[i][j+2][k] += ((4*error)/42);
					NewImagedata[i+1][j][k] += ((8*error)/42);
					NewImagedata[i+1][j+1][k] += ((4*error)/42);
					NewImagedata[i+1][j+2][k] += ((2*error)/42);
					NewImagedata[i+2][j][k] += ((4*error)/42);
					NewImagedata[i+2][j+1][k] += ((2*error)/42);
					NewImagedata[i+2][j+2][k] += ((1*error)/42);
					NewImagedata[i+1][j-2][k] += ((2*error)/42);
					NewImagedata[i+1][j-1][k] += ((4*error)/42);
					NewImagedata[i+2][j-1][k] += ((2*error)/42);
					NewImagedata[i+2][j-2][k] += ((1*error)/42);
				...
				for(int j=Sizew-3;j>=2;j--){								// repeating for odd rows
					...
					NewImagedata[i][j-1][k] += ((8*error)/42);
					NewImagedata[i][j-2][k] += ((4*error)/42);
					NewImagedata[i+1][j][k] += ((8*error)/42);
					NewImagedata[i+1][j+1][k] += ((4*error)/42);
					NewImagedata[i+1][j+2][k] += ((2*error)/42);
