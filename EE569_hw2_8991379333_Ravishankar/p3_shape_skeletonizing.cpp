...

{
	...
	unordered_map<int,bool> skeleton_1;
	
	for(int i=0;i<40;i++){


		skeleton_1[binary2int(Skeleton_stage1[i])] = true;
	}


	// Hashmap to map all binary masks to 1 for stage 2 filter
	unordered_map<int,bool> skeleton_2;
	
	for(int i=0;i<348;i++){
		
		skeleton_2[binary2int(Skeleton_stage2[i])] = true;
	}
		...
		for(int i=1;i<Sizeh-1;i++){
			for(int j=1;j<Sizew-1;j++){
				...
				if((int)G_matrix[i][j] == 1){							// Check if center is 1
					...
					if(skeleton_1[binary2int(block_matrix)] == true){ // Hit
...
// ---------------------------------------------------------------------------------------------------------------------------------------
		...
		for(int i=1;i<Sizeh-1;i++){
			for(int j=1;j<Sizew-1;j++){
				if((int)M_matrix[i][j] == 1){				// Check for centre 1
					...
					if(skeleton_2[binary2int(block_matrix)] == 0){  	// Miss
